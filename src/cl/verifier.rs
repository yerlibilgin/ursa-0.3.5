...
impl ProofVerifier {
  ...
  pub fn verify(&mut self, proof: &Proof, nonce: &Nonce) -> UrsaCryptoResult<bool> {
    ...
    for idx in 0..proof.proofs.len() {
      ...
      info!("Cred idx: {}, rev_reg: {:#?}", idx, (credential.rev_reg.as_ref()));
      if let (
    ...
    trace!(target: "anoncreds_service", "Verifier verify proof -> done");
  ...
  fn _verify_equality(
    ...
    trace!("ProofVerifier::_verify_equality: <<< t_equality: {:?}", t);
  ...
  fn _verify_non_revocation_proof(
    ...
    info!("Verifier::_verify_non_revocation_proof h {:#?}", &r_pub_key.h);
    info!("Verifier::_verify_non_revocation_proof htilde {:#?}", &r_pub_key.htilde);
    info!("Verifier::_verify_non_revocation_proof rhohat {:#?}", &proof.x_list.rho);
    info!("Verifier::_verify_non_revocation_proof ohat {:#?}", &proof.x_list.o);
    info!("Verifier::_verify_non_revocation_proof ch_num_z {:#?}", &ch_num_z);
    info!("Verifier::_verify_non_revocation_proof rev_reg.accum {:#?}", hex::encode(&rev_reg.accum.to_bytes()?));
    ...
    info!("Verifier::_verify_non_revocation_proof t1_hat: {:#?}", hex::encode(&non_revoc_proof_tau_list.t1.to_bytes()?));
    info!("Verifier::_verify_non_revocation_proof t2_hat: {:#?}", hex::encode(&non_revoc_proof_tau_list.t2.to_bytes()?));
    info!("Verifier::_verify_non_revocation_proof t3_hat: {:#?}", hex::encode(&non_revoc_proof_tau_list.t3.to_bytes()?));
    info!("Verifier::_verify_non_revocation_proof t4_hat: {:#?}", hex::encode(&non_revoc_proof_tau_list.t4.to_bytes()?));
    info!("Verifier::_verify_non_revocation_proof t5_hat: {:#?}", hex::encode(&non_revoc_proof_tau_list.t5.to_bytes()?));
    info!("Verifier::_verify_non_revocation_proof t6_hat: {:#?}", hex::encode(&non_revoc_proof_tau_list.t6.to_bytes()?));
    info!("Verifier::_verify_non_revocation_proof t7_hat: {:#?}", hex::encode(&non_revoc_proof_tau_list.t7.to_bytes()?));
    info!("Verifier::_verify_non_revocation_proof t8_hat: {:#?}", hex::encode(&non_revoc_proof_tau_list.t8.to_bytes()?));
...
mod tests {
  ...
  fn _doit() -> UrsaCryptoResult<u32> {
    let rho = GroupOrderElement::from_string("1B8A66D58ECE335C505B1FED48FD1FDC819C8F46D9377285A0B53DF4AD3945A9")?;
    let o = GroupOrderElement::from_string("19CD5B4942A89D597A0EA783017912747873312D5A829036A92767E5862BFFC0")?;
    let E = PointG1::from_string("6 4EC4943B825246D20061A0755DE9DC839DFB441B706F2BFB8502682929D2CC6D 4 \
        2FF5538EC61695536F972D189CC085BD1CB424E8B77C0F32E4A73265CA50D531 4 \
        205D7C73D3D82BC5B243E6ABD3EC039C055BDF5BCC3FBEA0AD3E549634F80E67")?;
    let h = PointG1::from_string("1 208670907A46D136A0796D1557E8BAA6C52284122FFC4B0B34D7CFE58DE751D0 1 \
        073875875520D8328FD2612EAD89BA24C43B59ECA4971DCDF5403C45199B97B2 2 \
        095E45DDF417D05FB10933FFC63D474548B7FFFF7888802F07FFFFFF7D07A8A8")?;
    let htilde = PointG1::from_string("1 0688C13EF9D0C8E0BD163C56F9451961AEB682576522DA96BEE0BEED37C10AA1 1 \
    17B5A3C4C9F8AA190A4311450421FA400AA3DDE6AF1889E629E2E67C160A80DD 2 095E45DDF417D05FB10933FFC63D474548B7FFFF7888802F07FFFFFF7D07A8A8")
      ?;

    let rhotilde = GroupOrderElement::from_string("1B89192936519908F7D3321983E8BFE923BB498260EB1B645EE2227654EAC25C")?;
    let otilde = GroupOrderElement::from_string("0F9516A0224BC0DE4BC5D06C971DB57F8D485F7E0424EE05DC96D6F93AB55422")?;
    let T1 = PointG1::from_string("6 3A2A97707E79018F6E6123A9BCC116DAF46B3C94B84C25D1E616BC052FC17CD9 4 2B8932E3FD37DD9BAD0645381F27F9EB43352408533DFFF9F644437D04534007 4 198F55D270C459D0C42C39C21A2732F4ADD7FCA6F0D71F146CC43A787FF03EED")?;
    //let nonce = BigNum::fromstring("5990970050");
    //let challenge = BigNum::fromstring("40643690060243635328011094887343158506262259992389248272784034400215731232570");

    let ch_num_z = GroupOrderElement::from_string("59DB836B94B524F68FF726649354446265728FDA5F876545378CA4340B1B5F3A")?;
    let rhohat = GroupOrderElement::from_string("1160961D80CE4FA57CB226341E5C5269770E8D88BFEE0D7679F9361EB1AE20F5")?;
    let ohat = GroupOrderElement::from_string("06FE2CBBC68C5D7C7647011D7D8712AEC577F472A7D48A46892100D1AEE7816C")?;
    let that = PointG1::from_string("6 46C4DD6AF8751CCA26D4922EC8B3B6204EB7C9FBF184F9E0F8320E85F9CED6E1 4 \
    413DC9C25F86ABC130DAA3F5D79A12500C0001D2F0D4102BE3B90DFB78D36EBC 4 1F5A456F388FCFF99CC2B9056D559318F20724A888CEC9F320B306AA35E4D3BD")?;


    //verify that hats are correct

    //x.add_mod(&ch_num_z.mul_mod(&y)?.mod_neg()?)?)
    let ruhat = rhotilde.add_mod(&ch_num_z.mul_mod(&rho)?.mod_neg()?)?;
    println!("ruhat {:#?}", &ruhat);
    assert_eq!(&ruhat, &rhohat);

    let eback = (h.mul(&rho)?).add(&htilde.mul(&o)?)?;
    println!("eback {:#?}", &eback);
    assert_eq!(&E, &eback);

    let T1_manual = (h.mul(&rhotilde)?.add(&htilde.mul(&otilde)?))?;
    println!("T1_manual: {:#?}", T1_manual);
    assert_eq!(&T1_manual, &T1);
    let diff = h.mul(&rhohat)?.add(&htilde.mul(&ohat)?)?;
    println!("Diff: ${:#?}", &diff);

    let that2 = (&E.mul(&ch_num_z)?).add(&diff)?;
    let that3 = (&E.mul(&ch_num_z)?).add(&h.mul(&rhohat)?)?.add(&htilde.mul(&ohat)?)?;

    let t1_slice = T1.to_bytes()?;
    let t1_manual_slice = T1_manual.to_bytes()?;
    let that_slice = that.to_bytes()?;
    let that2_slice = that2.to_bytes()?;
    let that3_slice = that3.to_bytes()?;

    let hash_t1_slice = get_hash_as_int(vec![t1_slice].as_slice())?;
    let hash_t1_manual_slice = get_hash_as_int(vec![t1_manual_slice].as_slice())?;
    let hash_that_slice = get_hash_as_int(vec![that_slice].as_slice())?;
    let hash_that2_slice = get_hash_as_int(vec![that2_slice].as_slice())?;
    let hash_that3_slice = get_hash_as_int(vec![that3_slice].as_slice())?;

    println!("{:#?}", &hash_t1_slice);
    println!("{:#?}", &hash_t1_manual_slice);
    println!("{:#?}", &hash_that_slice);
    println!("{:#?}", &hash_that2_slice);
    println!("{:#?}", &hash_that3_slice);
