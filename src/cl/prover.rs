...
impl ProofBuilder {
    ...
    pub fn finalize(&self, nonce: &Nonce) -> UrsaCryptoResult<Proof> {
        info!("ProofBuilder::finalize: >>> nonce: {:?}", nonce);
        ...
        info!("ProofBuilder::finalize challenge: {:#?}", challenge);
    ...
    fn _init_non_revocation_proof(
        ...
        info!("Prover::_init_non_revocation_proof rho: {:#?}", &c_list_params.rho);
        info!("Prover::_init_non_revocation_proof o: {:#?}", &c_list_params.o);
        info!("Prover::_init_non_revocation_proof E: {:#?}", &c_list.e);
        info!("Prover::_init_non_revocation_proof rev_reg.accum: {:#?}", hex::encode(&rev_reg.accum.to_bytes()?));
        ...
        info!("Prover::_init_non_revocation_proof h {:#?}", &cred_rev_pub_key.h);
        info!("Prover::_init_non_revocation_proof htilde {:#?}", &cred_rev_pub_key.htilde);
        info!("Prover::_init_non_revocation_proof rhotilde {:#?}", &tau_list_params.rho);
        info!("Prover::_init_non_revocation_proof otilde: {:#?}", &tau_list_params.o);

        info!("Verifier::_verify_non_revocation_proof t1: {:#?}", hex::encode(&tau_list.t1.to_bytes()?));
        info!("Verifier::_verify_non_revocation_proof t2: {:#?}", hex::encode(&tau_list.t2.to_bytes()?));
        info!("Verifier::_verify_non_revocation_proof t3: {:#?}", hex::encode(&tau_list.t3.to_bytes()?));
        info!("Verifier::_verify_non_revocation_proof t4: {:#?}", hex::encode(&tau_list.t4.to_bytes()?));
        info!("Verifier::_verify_non_revocation_proof t5: {:#?}", hex::encode(&tau_list.t5.to_bytes()?));
        info!("Verifier::_verify_non_revocation_proof t6: {:#?}", hex::encode(&tau_list.t6.to_bytes()?));
        info!("Verifier::_verify_non_revocation_proof t7: {:#?}", hex::encode(&tau_list.t7.to_bytes()?));
        info!("Verifier::_verify_non_revocation_proof t8: {:#?}", hex::encode(&tau_list.t8.to_bytes()?));
    ...
    fn _finalize_non_revocation_proof(
        ...
        info!("Prover::_finalize_non_revocation_proof finalize c_h {:#?}", &c_h);
        let ch_num_z = bignum_to_group_element(&c_h)?;
        info!("Prover::_finalize_non_revocation_proof ch_num_z {:#?}", &ch_num_z);
        ...
        info!("ProofBuilder::_finalize_non_revocation_proof rhohat: {:#?}", &non_revoc_proof.x_list.rho);
        info!("ProofBuilder::_finalize_non_revocation_proof ohat: {:#?}", &non_revoc_proof.x_list.o);
...
mod tests {
    ...
    */
    ...
    fn generate_proof_mocks() {
        ...
        info!("proof_request_nonce = {:#?}, ", proof_request_nonce);
